...
public class BiasStorageFilter extends AbstractStorageFilter<BiasContainer> {

	private final double pT = 0.1;
	private int dataSize;
	
	public BiasStorageFilter(final char c, final int dataSize) {
		super(c);
	...
	private int[][][] getCounts(final int genomicPosition, FilterContainer[] replicateFilterContainer) {
		...
		for (int replicateI = 0; replicateI < n; ++replicateI) {
			...
			BiasContainer biasContainer = getData(filterContainer);
